    ...
    if ($row = $result->fetch_assoc()) {
        if ($row['photo']) {
            ...
            $paths_to_check = [
                ...
                $base_path . '/users/img/' . $row['photo'],
                $base_path . '/users/img/mini/' . $row['photo'],
                // Standard paths (previous attempts)
                $base_path . '/user/' . $row['photo'],
                $base_path . '/user/mini/' . $row['photo'],
            ...
            foreach ($paths_to_check as $path) {
                ...
                if (file_exists($path)) {
                    ...
                    if (strpos($path, '/users/img/mini/') !== false) {
                        $photo_url = '/users/img/mini/' . $row['photo'];
                    } else if (strpos($path, '/users/img/') !== false) {
                        $photo_url = '/users/img/' . $row['photo'];
                    } else if (strpos($path, '/user/mini/') !== false) {
                        $photo_url = '/user/mini/' . $row['photo'];
                    } else if (strpos($path, '/user/') !== false) {
                        $photo_url = '/user/' . $row['photo'];
                    } else if (strpos($path, '/users/img/') !== false) {
                        $photo_url = '/users/img/' . $row['photo'];
                    } else {
                        // Use direct path
